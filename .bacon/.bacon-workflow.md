...
flowchart TD
    ...
    STRAT -->|risks acceptable| LINT[spec-lint.ps1]
    STRAT -->|risks too high| EXIT([Exit])
    ...
    CODER -->|in-progress| LOOP{check-fast.ps1}
    LOOP -->|pass| GATE2{User approve diff?}
    LOOP -->|fail, retry < 3| CODER
    LOOP -->|fail, retry = 3| HUMAN

    GATE2 -->|yes| AUDITOR{Auditor}
    GATE2 -->|no| EXIT
    GATE2 -->|--auto| AUDITOR

    AUDITOR -->|PASS| DONE[Move to _done/]
    AUDITOR -->|FAIL| HUMAN
...
|------|-------|--------|------|
| Observer | prompt or `_active/` scan | problem description | must find a valid target |
| Strategist | observer output | spec package in `_active/` | `Rust internal linting` |
| Coder | spec in `_active/` | code changes, status=`implemented` | `Rust temp-worktree check` (retry < 3 → CODER, retry = 3 → HUMAN) |
| Auditor | implemented spec | `_done/` or `needs-human-approval` | semantic LLM review |
...
1. **Detection**: Bacon scans `_active/` for `status: in-progress` on startup
2. **Notification**: User warned about incomplete specs
3. **Options**:
   - Resume: `bacon --stage <role> --spec <NN>`
   - Abandon: Move spec to `_abandoned/` manually
   - Reset: Delete spec and start over

### Error Logging
All errors are logged with context:
- `sessions/errors.json`: Structured error log
- `sessions/<role>_stderr.log`: Role-specific stderr
- `sessions/crash_dump_<timestamp>.json`: Crash state dump

## Retry with Error Feedback

When `check-fast.ps1` fails, the Coder feeds stderr/stdout back to the LLM and retries (up to 3 retries / 4 total attempts). Each retry includes the previous failure output so the LLM can fix specific errors. If all 4 attempts fail, the spec is marked `needs-human-approval`.
