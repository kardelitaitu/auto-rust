...
main() {
    ...
    local approved_patch="${SESSIONS_DIR}/approved_patches/verified_${rollback_id}.diff"
    if queue_verified_patch "$shadow_dir" "$rollback_id"; then
        log_info "SUCCESS: Patch verified and queued"
        if [[ "${BACON_AUTO_APPLY:-false}" == "true" ]]; then
            if [[ "${BACON_REQUIRE_FULL_CHECK:-true}" != "true" ]]; then
                log_error "Auto-apply requires BACON_REQUIRE_FULL_CHECK=true"
                return 1
            fi
            log_info "Auto-apply requested; applying queued patch with full check gate"
            "${SCRIPT_DIR}/bacon-apply-approved.sh" "$approved_patch" --run-check
        fi
        return 0
    else
