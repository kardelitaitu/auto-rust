    ...
    cat > "${audit_result}.quality" << EOF
{
  ...
  "warnings": $(printf '%s\n' "${quality_warnings[@]}" | jq -R . | jq -s .),
  "score": $([ "$quality_issues" -eq 0 ] && echo "100" || [ "$quality_issues" -le 2 ] && echo "80" || echo "60")
}
EOF
    ...
    cat > "${audit_result}.performance" << EOF
{
  ...
  "warnings": $(printf '%s\n' "${performance_warnings[@]}" | jq -R . | jq -s .),
  "impact": $([ "$performance_issues" -eq 0 ] && echo "\"low\"" || [ "$performance_issues" -le 2 ] && echo "\"medium\"" || echo "\"high\"")
}
EOF
    ...
    cat > "${audit_result}.browser" << EOF
{
  ...
  "warnings": $(printf '%s\n' "${browser_warnings[@]}" | jq -R . | jq -s .),
  "compatibility": $([ "$browser_issues" -eq 0 ] && echo "\"compatible\"" || echo "\"needs_review\"")
}
EOF
...
compile_test() {
    ...
    local test_dir="/tmp/bacon_audit_test_$(date +%s)"
    mkdir -p "$test_dir"
    
    # Copy source files
    if ! cp -r "$PROJECT_ROOT/src" "$test_dir/" 2>/dev/null; then
        log_error "Failed to copy source files for testing"
        rm -rf "$test_dir"
        return 1
    fi
    ...
    if [[ -f "$PROJECT_ROOT/Cargo.toml" ]]; then
        cp "$PROJECT_ROOT/Cargo.toml" "$test_dir/"
    fi
    ...
    if cargo check 2>/dev/null >/dev/null; then
    ...
    if [[ "$compile_result" == "success" ]]; then
        if cargo test --lib 2>/dev/null >/dev/null; then
            test_result="success"
