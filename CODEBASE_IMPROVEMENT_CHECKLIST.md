...
- [x] **Database Segmentation / Migration:**
  - Replace the monolithic JSON database (`twitter_activity_v2.json`) with an SQLite database or segment database files per browser profile to prevent file-locking contention. (Implemented profile-segmented JSON persistence in `twitteractivity_persistence.rs`, June 2026)
