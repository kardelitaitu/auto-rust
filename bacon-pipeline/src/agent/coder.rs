...
use crate::core::{collect_source_context, read_role_prompt, GitSnapshot};
...
fn plan_spec_code_paths(spec_path: &Path) -> Vec<String> {
    ...
    crate::core::extract_repo_file_refs(&spec_text)
        .into_iter()
        .filter(|path| {
...
fn run_check_fast(root: &Path) -> Result<String> {
    let check_fast_cmd = &crate::config::project_config().validation.check_fast;
    let output = Command::new(&check_fast_cmd[0])
        .args(&check_fast_cmd[1..])
        .current_dir(root)
        .output()
...
fn is_refusal(response: &str) -> bool {
    crate::core::is_refusal(response)
}
...
pub async fn run(llm: &crate::llm::Llm, args: &RunArgs, ctx: &PipelineCtx) -> Result<PipelineCtx> {
    ...
    let mut extracted_confidence: Option<crate::core::Confidence> = None;
    ...
    loop {
        ...
        let root = &crate::config::manifest_dir();
        ...
        if let Some(conf) = crate::core::extract_confidence(&response) {
            info!(
            ...
            match run_check_fast(root) {
                Ok(ref check_output) => {
                    ...
                    let config_auto_apply =
                        crate::core::PipelineConfig::from_bacon_toml().enable_auto_apply;
...
mod tests {
    ...
    fn test_refusal_all_phrases_detected() {
        for phrase in crate::core::REFUSAL_PHRASES {
            assert!(
                is_refusal(phrase),
...
"#;
    ...
    #[test]
    #[ignore = "requires host project filesystem — collect_source_context reads files from disk"]
    ...
    #[test]
    #[ignore = "requires host project filesystem — suggest_existing_rust_target reads files from disk"]
...
"#,
        ...
        assert!(msg.contains("src/agent/auditor.rs"));
        Ok(())
    }
