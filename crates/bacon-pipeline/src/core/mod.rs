...
//!
//! Shared pipeline types and utility functions for the bacon-pipeline crate.
...
impl std::fmt::Debug for PipelineCtx {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PipelineCtx")
            ...
            .field("stage_durations", &self.stage_durations)
            .finish()
    }
}
...
/// Validate that Bacon is running with a supported LLM provider.
pub fn validate_bacon_local_only() -> Result<()> {
    ...
    if llm_config.exists() {
        ...
        {
            anyhow::bail!("Bacon requires config/llm.toml provider = \"nvidia\" or \"ollama\"");
        }
    }
        ...
        if let Some(agents) = table.get("agents").and_then(|v| v.as_table()) {
            for (name, cfg) in agents {
                let provider = cfg
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("nvidia");
                let allowed = provider.eq_ignore_ascii_case("nvidia")
                    || provider.eq_ignore_ascii_case("ollama")
                    || provider.eq_ignore_ascii_case("cli");
                if !allowed {
                    anyhow::bail!(
                        "Bacon agent '{name}' uses unsupported provider '{provider}'; use nvidia, ollama, or cli"
                    );
                }
            }
        }
...
pub fn check_stale_in_progress() -> Result<()> {
    ...
    for entry in &entries {
        ...
        if status == "in-progress" {
            ...
            // Auto-recovery: if spec.yaml is older than 30 minutes,
            // reset status to "approved" so the pipeline can retry.
            // NOTE: Use spec.yaml mtime (not directory mtime) — directory
            // timestamps on Windows do not reliably update when child files change.
            let metadata = if let Ok(m) = std::fs::metadata(&meta_path) {
                m
            } else {
...
pub fn read_role_prompt(role: &str) -> String {
    ...
    let path = crate::config::project_config().roles_dir.join(file);
...
/// Get the project root at runtime (configurable via `ProjectConfig`).
fn manifest_dir() -> PathBuf {
    crate::config::manifest_dir()
}

/// Resolve Bacon pipeline config, allowing integration tests to isolate routing.
fn bacon_config_path() -> PathBuf {
    crate::config::bacon_config_path()
}
...
mod tests {
    ...
    #[test]
    #[ignore = "requires host project filesystem — run with BACON_PROJECT_ROOT set"]
    ...
    #[test]
    #[ignore = "requires host project filesystem — reads source files from disk"]
    ...
    fn collect_source_context_handles_windows_path_separators() {
        let text = r#"
            Review src/agent/coder.rs and src/core/mod.rs.
        "#;
        let context = collect_source_context(text, 10, 50);

        assert!(context.contains("## Relevant Source Files"));
        assert!(context.contains("src/agent/coder.rs"));
        assert!(context.contains("src/core/mod.rs"));
    }

    #[test]
    fn count_spec_file_refs_handles_repo_relative_paths_and_backslashes() {
        let text = r#"
            Update src\bin\bacon.rs and src/core/mod.rs.
    ...
    #[test]
    #[ignore = "requires host project filesystem — validate_spec_file_refs reads plan.md from disk"]
    fn validate_spec_file_refs_normalizes_windows_paths_and_keeps_existing_files() {
        let temp_dir = tempfile::tempdir().expect("temp dir");
        std::fs::write(
            temp_dir.path().join("plan.md"),
            r#"
                Update src\core\mod.rs and docs/specs/README.md.
