...
- [x] LLM client can parse both NVIDIA and Ollama response formats based on provider config
- [x] `--parallel` flag spawns independent specs as concurrent tokio tasks

### Phase 4: Observability
- [x] `.bacon/sessions/run-log.json` exists after each pipeline run
- [x] Run log contains timestamp, spec_id, per-stage duration, and outcome
- [x] `--ci` flag emits GitHub Actions-compatible JSON annotations
- [x] Non-goals explicitly documented and enforced
