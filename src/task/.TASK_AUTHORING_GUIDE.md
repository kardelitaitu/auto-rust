...
context.navigate(url: &str, timeout_ms: u64) -> Result<()>  // Navigate to URL
```

### Element Interaction Methods

```rust
context.click(selector: &str) -> Result<ClickOutcome>           // Click an element
context.double_click(selector: &str) -> Result<ClickOutcome>     // Double-click
context.right_click(selector: &str) -> Result<ClickOutcome>      // Right-click
context.hover(selector: &str) -> Result<ClickOutcome>           // Hover over element
...
### Timeout Message Guidelines

When a `TaskContext` wrapper times out, label the error with:

```text
wrapper_timeout | stage=<stage_name> ...
```

Rules:

- Use a stable `stage` name like `navigate_timeout`, `wait_for_load`, `wait_for_visible`, or `click_total`.
- Include the most useful context, such as `selector`, `url`, `timeout_ms`, or `timeout_secs`.
- Keep wrapper errors distinct from task-level policy timeouts.
- Avoid generic `deadline has elapsed` messages at the API boundary.
...
| `allow_export_cookies` | Read browser cookies | `context.export_cookies_for_domain()` |
...
Here's a complete task that demonstrates using multiple v0.1.1 APIs:
...
pub async fn data_extract(ctx: &TaskContext) -> Result<()> {
    ...
    ctx.navigate("https://shop.example.com/products").await?;
...
async fn test_data_extract_mock() {
    ...
    // (MockTaskContext removed — use real TaskContext with test page fixture)
...
- [API Reference](../docs/API_REFERENCE.md) - Full TaskContext API documentation
- [Configuration](../docs/ARCHITECTURE.md) - Config file structure and validation
