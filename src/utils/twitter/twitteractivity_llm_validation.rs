...
pub fn validate_reply(text: &str) -> Result<String> {
    ...
    // Remove any <unused\d+> tokens generated by the LLM, and trim again
    sanitized = remove_unused_tokens(&sanitized).trim().to_string();
...
fn unused_tokens_regex() -> &'static regex::Regex {
    static RE: OnceLock<regex::Regex> = OnceLock::new();
    RE.get_or_init(|| compile_regex(r"<unused\d+>"))
}

/// Removes <unused\d+> tokens from text, handling both complete and partial trailing tokens.
fn remove_unused_tokens(text: &str) -> String {
    let cleaned = unused_tokens_regex().replace_all(text, "").to_string();

    // Also remove any partial trailing unused token if the response was truncated
    static PARTIAL_RE: OnceLock<regex::Regex> = OnceLock::new();
    let partial_re =
        PARTIAL_RE.get_or_init(|| compile_regex(r"<unused\d*$|<unuse$|<unus$|<unu$|<un$|<u$|<$"));

    partial_re.replace_all(&cleaned, "").to_string()
}
...
```"#;
    ...
    #[test]
    fn test_validate_reply_removes_unused_tokens() {
        let text = "<unused25>this is <unused0>a clean reply <unused145>";
        let result = validate_reply(text).unwrap();
        assert_eq!(result, "this is a clean reply");

        // Test with a trailing partial token
        let text_partial = "<unused25>this is <unused0>a clean reply <un";
        let result_partial = validate_reply(text_partial).unwrap();
        assert_eq!(result_partial, "this is a clean reply");
    }
