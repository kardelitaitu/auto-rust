...
async fn monitor_health(
    ...
    loop {
        ...
        if health_stats.health_score < 50 {
            warn!("[Soak Test] Health score below 50: {}", health_stats.health_score);
        }
...
async fn test_extended_soak() {
    ...
    info!("Success rate: {:.1}%", (success_count as f64 / task_count as f64) * 100.0);
    info!("Tasks per second: {:.2}", task_count as f64 / elapsed.as_secs_f64());
    info!("");
    info!("Final Metrics:");
    ...
    info!("  Consecutive failures: {}", health_stats.consecutive_failures);
    info!("  Total failures: {}", health_stats.total_failures);
    info!("  Uptime: {:.2?}", Duration::from_millis(health_stats.uptime_ms));
    info!("");
    
    // Assertions for test pass/fail
    assert!(task_count > 1000, "Should have executed at least 1000 tasks");
    assert!(metrics.success_rate() > 90.0, "Success rate should be above 90%");
    assert!(health_stats.health_score > 50, "Health score should remain above 50");
